 ...
 *
 * <p>요청자의 {@link Member} 정보를 조회한 뒤, 해당 회원의 역할이 {@link MemberRole#MASTER_MANAGER}
 * 인지 여부를 통해 마스터 권한 보유 여부를 판단한다. 도메인 계층에서는 {@link PermissionChecker}
 * 인터페이스에만 의존하며, 이 구현체는 인프라스트럭처 계층에서 실제 조회 로직을 제공한다.
 *
 * @author 사용자
 * @since 1.0.0
 */
...
public class PermissionCheckerImpl implements PermissionChecker {
   ...
   *
   * <p>먼저 요청자의 ID로 {@link Member} 를 조회하고, 회원이 존재하지 않을 경우
   * {@link MemberException} 을 발생시킨다. 이후 요청자의 역할이 {@link MemberRole#MASTER_MANAGER}
   * 인지 여부를 반환한다.
   *
   * @param requesterId 권한을 확인할 요청자(회원)의 UUID
   * @param targetMemberId (옵션) 권한이 행사될 대상 회원의 ID. 현재 구현에서는 사용하지 않지만,
   *     향후 대상 회원에 따른 세부 권한 검증이 필요할 경우를 대비해 포함되어 있다.
   * @return 요청자의 역할이 {@link MemberRole#MASTER_MANAGER} 이면 {@code true}, 그렇지 않으면
   *     {@code false}
