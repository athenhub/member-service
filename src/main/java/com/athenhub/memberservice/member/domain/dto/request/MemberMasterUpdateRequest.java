 ...
 *
 * <p>회원의 이름, 역할, 소속 유형 및 소속명, 상태를 변경할 수 있으며, 필수 값에 대해서는 Bean Validation 애너테이션을 통해 유효성이 검증된다.
 *
 * <ul>
 *   <li>{@code name} — 회원 이름 (필수)
 *   <li>{@code role} — 회원 역할(권한) 정보 (필수)
 *   <li>{@code organizationType} — 회원이 속한 조직 유형 (필수)
 *   <li>{@code organizationName} — 회원이 속한 조직 이름 (선택)
 *   <li>{@code status} — 회원 상태(예: PENDING, ACTIVE, INACTIVE 등) (필수)
 * </ul>
 *
 * @author 박성준
 * @since 1.0.0
